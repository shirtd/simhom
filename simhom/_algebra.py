...
class FGGroup(Basis, Group):
    ...
    def __contains__(self, g):
        if (not len(self.basis)
            or any(not e in self.imap for e in g)):
            return False
        if self.zero.subgroup is not None:
            zmat = hstack(self.to_vec(e) for e in self.zero.basis)
            M = hstack(self.to_mat(), zmat, self.to_vec(g))
            return all(p < len(self.basis) + len(self.zero.basis) for p in M.rref()[1])
        else:
            M = hstack(self.to_mat(), self.to_vec(g))
            return all(p < len(self.basis) for p in M.rref()[1])
    def __repr__(self):
